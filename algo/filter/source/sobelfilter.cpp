...
int sobelFilter::execute(char* inputImgPath, char* outputImgPath)
{
...
int sobelFilter::xGradient(cv::Mat img, int rowNo, int colNo)
{
	int xGrad = img.at<uchar>(rowNo - 1, colNo - 1) +
		(2 * img.at<uchar>(rowNo, colNo - 1)) +
...
int sobelFilter::yGradient(cv::Mat img, int rowNo, int colNo)
{
	int yGrad = img.at<uchar>(rowNo - 1, colNo - 1) +
				(2*img.at<uchar>(rowNo - 1, colNo)) +
				img.at<uchar>(rowNo - 1, colNo + 1) -
				img.at<uchar>(rowNo + 1, colNo - 1) -
				(2*img.at<uchar>(rowNo + 1, colNo)) -
				img.at<uchar>(rowNo + 1, colNo + 1);
